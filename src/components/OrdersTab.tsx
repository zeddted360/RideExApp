...
import { IBookedOrderFetched, IDiscountFetched, IFeaturedItemFetched, IMenuItemFetched, IPopularItemFetched, IUserFectched, OrderStatus } from "../../types/types";
...
export default function OrdersTab({
  ...
  const { currentExtra, loading: extraLoading } = useSelector((state: RootState) => state.extra);

  const [customerNames, setCustomerNames] = useState<{ [key: string]: string }>({});
  const [fetchingNames, setFetchingNames] = useState(false);
  const [selectedOrder, setSelectedOrder] = useState<IBookedOrderFetched | null>(null);
  const [selectedOrderToDelete, setSelectedOrderToDelete] = useState<IBookedOrderFetched | null>(null);
  const [fetchedExtras, setFetchedExtras] = useState<{ [key: string]: IFetchedExtras }>({});
  ...
  const findItemById = (id: string): ItemWithBucket => {
    let item: IMenuItemFetched | IFeaturedItemFetched | IPopularItemFetched | IDiscountFetched | undefined = menuItems.find((item) => item.$id === id);
    if (item) {
  ...
  useEffect(() => {
    if (selectedOrder && selectedOrder.items) {
      try {
        ...
        const fetchExtraPromises = Array.from(extraIds).map((extraId) =>
          dispatch(fetchExtraById(extraId))
            .unwrap()
            .then((extra) => {
              setFetchedExtras((prev) => ({ ...prev, [extra.$id]: extra }));
            })
            .catch((err) => {
              console.error(`Failed to fetch extra ${extraId}:`, err);
            })
        );

        Promise.all(fetchExtraPromises).finally(() => setFetchingExtras(false));
      } catch (err) {
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      ...
      preparing: "bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800",
      ready: "bg-indigo-100 text-indigo-800 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-800",
      out_for_delivery: "bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-800",
    ...
    <>
      ...
      <div className="mb-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-50 to-pink-50 dark:from-orange-900/20 dark:to-pink-900/20 rounded-xl border border-orange-200 dark:border-orange-800 shadow-sm">
            ...
            <span className="font-bold text-gray-900 dark:text-white">{filteredOrders.length}</span>
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(5)].map((_, index) => (
            <div key={index} className="animate-pulse bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-900 dark:text-gray-100">{customerName}</span>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-2">
          ...
          <div className="lg:hidden space-y-4">
            {paginatedOrders.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="space-y-3 mb-4">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-900 dark:text-gray-100">{branch ? branch.name : "-"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm">
                        <User className="w-4 h-4 text-gray-400 flex-shrink-0" />
              ...
              <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium text-center">No orders found</p>
                <p className="text-gray-400 dark:text-gray-500 text-sm text-center mt-1">Try adjusting your filters</p>
              </div>
            )}
          </div>
          ...
          {filteredOrders.length > ordersPerPage && (
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8 p-4 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700">
              ...
              <span className="text-gray-700 dark:text-gray-300 font-medium">
                Page <span className="font-bold text-orange-600 dark:text-orange-400">{currentPage}</span> of{" "}
                <span className="font-bold">{Math.ceil(filteredOrders.length / ordersPerPage)}</span>
              </span>
              <Button
                onClick={() =>
                  setCurrentPage(
          ...
          <div className="space-y-6 p-4 sm:p-6">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 sm:p-5 rounded-2xl border-2 border-blue-200 dark:border-blue-800">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">Delivery Information</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="flex items-start gap-2 p-3 bg-white dark:bg-gray-800 rounded-xl border border-blue-200 dark:border-blue-800">
                    ...
                    <button
                      onClick={() => deliveryAddress && handleCopy(deliveryAddress, "Address")}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 p-3 bg-white dark:bg-gray-800 rounded-xl border border-blue-200 dark:border-blue-800">
                    ...
                    <button
                      onClick={() => deliverContact && handleCopy(deliverContact, "Contact")}
                      className="p-1.5 rounded-lg bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors"
            ...
            <div className="bg-gradient-to-br from-orange-50 to-pink-50 dark:from-orange-900/20 dark:to-pink-900/20 p-4 sm:p-5 rounded-2xl border-2 border-orange-200 dark:border-orange-800">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">Order Summary</h3>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3 sm:gap-x-6 text-sm">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Items:</span>
                  <span className="font-bold text-gray-900 dark:text-white">{structuredItems.length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Total Qty:</span>
                  <span className="font-bold text-gray-900 dark:text-white">
                    {structuredItems.reduce((sum, item) => sum + item.quantity, 0)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Delivery Fee:</span>
                  <span className="font-semibold text-orange-600 dark:text-orange-400">₦{deliveryFee.toLocaleString()}</span>
                </div>
            ...
            {/* Order Items */}
            {fetchingExtras || extraLoading === "pending" ? (
              <div className="flex flex-col items-center justify-center py-12">
                <Package className="w-12 h-12 animate-spin text-orange-500 mb-4" />
                <p className="text-gray-600 dark:text-gray-400 font-medium">Loading order details...</p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {structuredItems.map((structuredItem, itemIndex) => {
                  const { item, bucketId } = findItemById(structuredItem.itemId);

                  if (!item) {
                    return (
                      <div key={itemIndex} className="p-5 bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 rounded-2xl border-2 border-red-200 dark:border-red-800">
                        <div className="flex items-center gap-3">
                          <div className="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center">
                            <Package className="w-8 h-8 text-gray-400" />
                          </div>
                          <div>
                            <h4 className="font-semibold text-gray-900 dark:text-white">Item Not Found</h4>
                            <p className="text-sm text-red-500">ID: {structuredItem.itemId}</p>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  return (
                    <div key={itemIndex} className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border-2 border-gray-200 dark:border-gray-700 overflow-hidden">
                      {/* Item Header */}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              <span className="text-base font-bold text-orange-600 dark:text-orange-400">
                                ₦{structuredItem.priceAtOrder.toLocaleString()} each
                              </span>
                            </div>
                          </div>
      ...
      <Dialog open={!!selectedOrderToDelete} onOpenChange={() => setSelectedOrderToDelete(null)}>
        <DialogContent className="sm:max-w-md bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700">
          ...
          <div className="p-6 pt-0">
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              Are you sure you want to delete order #{selectedOrderToDelete?.riderCode?.toUpperCase() || selectedOrderToDelete?.orderId}? This action cannot be undone.
            </p>
          </div>
